...
await page.locator('#create-figure').click()
await page.locator('.fe-page svg').waitFor()
const figureInitialRows = await page.locator('.fe-sidebar-section').filter({ hasText: 'Track rows' }).locator('.fe-layer-line').count()
await page.locator('.figure-editor [data-action="add-column"]').click()
await page.locator('.fe-page svg [data-fe-column]').first().waitFor()
const figureColumns = await page.locator('.fe-sidebar-section').filter({ hasText: 'Columns' }).locator('.fe-layer-line').count()
const secondFigureColumn = await page.locator('.fe-sidebar-section').filter({ hasText: 'Columns' }).locator('.fe-layer-line').nth(1).locator('[data-select="column"]').getAttribute('data-column')
await page.locator(`.fe-sidebar [data-action="adjust-region"][data-column="${secondFigureColumn}"]`).click()
await page.locator('.fe-region-dialog .fe-crop-preview svg').waitFor()
await page.locator('.fe-region-dialog [data-crop="start"]').fill('35000000')
await page.locator('.fe-region-dialog [data-crop="end"]').fill('35010000')
await page.locator('.fe-region-dialog [data-crop="apply"]').click()
await page.locator('.fe-sidebar [data-select="row"]').first().click()
await page.locator('.fe-inspector [data-edit="row-label"]').fill('Figure genes')
await page.locator('.fe-inspector [data-edit="row-label"]').press('Tab')
const figureEditedLabel = await page.locator('.fe-sidebar [data-select="row"]').first().textContent()
await page.locator('.figure-editor [data-action="undo"]').click()
const figureUndoLabel = await page.locator('.fe-sidebar [data-select="row"]').first().textContent()
await page.locator('.figure-editor [data-action="redo"]').click()
const figureRedoLabel = await page.locator('.fe-sidebar [data-select="row"]').first().textContent()
const [figureProjectDownload] = await Promise.all([page.waitForEvent('download'), page.locator('.figure-editor [data-action="save"]').click()])
const figureProjectPath = await figureProjectDownload.path()
const figureProject = JSON.parse(await readFile(figureProjectPath, 'utf8'))
const [figureProjectChooser] = await Promise.all([page.waitForEvent('filechooser'), page.locator('.figure-editor [data-action="open"]').click()])
await figureProjectChooser.setFiles(figureProjectPath)
await page.locator('.fe-sidebar [data-select="row"]').first().filter({ hasText: 'Figure genes' }).waitFor()
const [figureSvgDownload] = await Promise.all([page.waitForEvent('download'), page.locator('.figure-editor [data-action="export-svg"]').click()])
const figureSvg = await readFile(await figureSvgDownload.path(), 'utf8')
await page.locator('.figure-editor [data-action="export-png"]').click()
const [figurePngDownload] = await Promise.all([page.waitForEvent('download'), page.locator('.fe-confirm button[value="300"]').click()])
const figurePng = await readFile(await figurePngDownload.path())
await page.screenshot({ path: 'dist/figure-editor-smoke.png' })
await page.locator('.figure-editor [data-action="back"]').click()
const figureResumeVisible = await page.locator('#create-figure').textContent()
if (figureInitialRows < 1 || figureColumns !== 2 || !figureEditedLabel?.includes('Figure genes') || figureUndoLabel?.includes('Figure genes') || !figureRedoLabel?.includes('Figure genes') || !figureResumeVisible?.includes('Continue figure') || figureProject.schemaVersion !== 1 || figureProject.columns.length !== 2 || !figureSvg.includes('<svg') || !figureSvg.includes('Figure genes') || figurePng.subarray(0, 8).toString('hex') !== '89504e470d0a1a0a') throw new Error('Figure Editor handoff, project round trip, edit, and export smoke failed')
...
await page.waitForFunction(() => document.querySelector('#locus-input')?.value === 'chr1:109,681,277-110,058,784')
...
if (dataPaths.length > 0) {
  ...
  await page.locator('.fe-page svg').waitFor()
  const realFigureRows = await page.locator('.fe-sidebar [data-select="row"]').count()
