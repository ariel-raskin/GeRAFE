...
- Cloud-file progress depends on the Windows offline/recall file attributes. When the Windows Cloud Files API exposes locally available bytes, the in-row percentage is labeled **local**; otherwise GeRAFE falls back to bytes it has read, labeled **prepared**. Neither value is a provider-reported network-transfer percentage. Some providers block while fetching the entire file on the first read, so progress may remain indeterminate until that read completes. Large online-only genomics files may require substantial local storage and time. Browser sessions do not expose these native file attributes.
