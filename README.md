...
An optional matrix cursor outlines the hovered cell and reports both genomic bins, their separation, the contact value or data state, resolution, normalization, and transformation. Matrix settings independently control the color-scale legend, track-card metadata, sparse zero fill, explicit missing/NaN pixels, and normalization-masked bins. Masked-bin display is available when the active file reader exposes the required normalization mask.
