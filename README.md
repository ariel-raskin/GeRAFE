...
GeRAFE reads genomic coordinates from each Cooler's stored bin table and canonicalizes the triangle orientation of both Cooler and `.hic` cells before drawing. Matrix readers and their indexes are reused across queries, pans use an overscanned window, and contacts too distant to be visible within the track height are omitted from the render payload to keep navigation responsive. Auto-z statistics and 64 visually smooth intensity levels are prepared once into a memory-bounded offscreen raster for each loaded matrix payload. Panning inside that payload then crops a single cached image rather than replaying every contact on every frame. Only contacts returned by the sparse matrix source are painted; absent, zero, masked, or unavailable bins retain the canvas background because the current sparse payload does not reliably distinguish among them.
