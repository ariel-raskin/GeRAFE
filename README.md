...
For a desktop-opened matrix, right-click and choose **Derive signal track → Insulation** or **Compartment PC1**. Both calculate from chromosome-wide observed/expected contacts, rather than the current viewport, and cache by file timestamp. Insulation uses a diamond of at least two bins on either side of each boundary (targeting a 500 kb window), measures mean crossing contacts including sparse zeros, and plots log2 enrichment relative to the chromosome-wide baseline; masked or explicitly missing pixels are excluded. PC1 is the leading component of the chromosome-wide contact-profile correlation at a resolution with at most 1,000 bins; its positive/negative sign is mathematically arbitrary, not a labeled A/B compartment without external annotations. These tracks store the original file path and calculation choices, not precomputed data. A different file or normalization calls for a new derived track; no matrix-derived track is exported as a BigWig.

The matrix inspector outlines the hovered cell and, by default, shows only its contact value or data state. Under **Settings → Track behavior → Matrix tracks**, the inspector can be turned off globally, its value, interacting-bin, and detail lines can be shown independently, and matrix color scales or track-card metadata can be hidden across the app. Per-track Matrix settings control sparse zero fill, explicit missing/NaN pixels, and normalization-masked bins. Masked-bin display is available when the active file reader exposes the required normalization mask.

GeRAFE reads genomic coordinates from each Cooler's stored bin table and canonicalizes the triangle orientation of both Cooler and `.hic` cells before drawing. Matrix readers and their indexes are reused across queries, and pans use an overscanned window. New tracks query the full visible genomic span by default; a bounded automatic-depth option remains available. Neither mode depends on track height, so resizing a track cannot silently multiply its contact query. Contacts are drawn directly at full canvas resolution. Optional zero fill is one clipped background shape rather than a dense set of generated cells, preserving the sparse rendering path.
...
- Contact-matrix tracks currently show one-dimensional cis windows as triangular heatmaps. Interchromosomal maps, two-axis navigation, matrix comparisons, and matrix-derived annotations are not yet available.
