...
- Reversible signal stacks combine two or more ordinary quantitative tracks in one shared-scale row, with smooth equal-weight curves, color/line-pattern differentiation, a synchronized member legend, and per-member order and visibility.
...
- In the Windows desktop app, all file open/save commands use GeRAFE's own file browser, including tracks, workspaces, IGV sessions, reference indexes, and BAM indexes. Folder browsing does not ask the system Open dialog to materialize cloud files first. Click any path segment to jump to that folder, enter a path, choose a drive, or save frequently used folders as quick buttons. Supported files show **Online-only** when Windows reports offline/recall attributes, or **On this device** otherwise; **Refresh** updates this snapshot without opening file contents. For tracks, an **In workspace** badge marks files referenced by current tracks, including pending ones; BAM indexes show **Index in use**. These badges match full paths and do not prevent opening a file again. Filter filenames and select multiple tracks before **Open selected**. The track browser remembers the last successfully opened track folder independently of the workspace folder. An online-only file reserves its normal track position immediately, showing its name and an in-row progress bar until the data appears. BAM indexes are prepared too.
- Search for a gene such as `RUNX1`, or paste coordinates such as `chr8:127,700,001-127,900,000` or `CHR8 127700001 127900000`. The search accepts extra spaces or tabs, optional thousands commas, and `-`, typographic dashes, `..`, or `to` between positions. All entered coordinates use 1-based inclusive positions, including three-column input (not BED's 0-based convention).
...
Use the compact **Regions** menu beside **Fit tracks** to **Add region…** by dragging across the ruler or track area, or **Add current view as region…** directly. Holding `Ctrl` temporarily enables selection for the next left-button drag in the genomic plot. Saved regions keep a name, highlight color, genomic interval, and show/hide state. Their appearance panel chooses dashed, solid, or no boundary; fill on/off; and shade intensity, with the existing highlight updating live as the shade slider moves. This supports outline-only, fill-only, combined, or fully hidden interiors without drawing horizontal edges along track boundaries. Click a saved region to return to it; the adjacent controls toggle its highlight, show and edit its current color, rename it, or remove it. On ordinary tracks, hover either visible vertical boundary for the horizontal-resize cursor, then drag it to resize the saved interval. On a triangular cis matrix, the same cursor and drag action are available only on the two visible slanted sides of the region triangle. Panning translates region highlights and manual matrix outlines in genomic coordinates; annotations pass beneath the viewport edge and are clipped instead of being resized to the visible window. A matrix outline's upper geometry remains visible after either anchor leaves the base axis, until that geometry exits the rendered depth band. Manual matrix outlines also remain visible while an assigned matrix source is loading or offline.
...
- To compare ordinary BigWig, bedGraph, or TDF signals directly, select two or more compatible tracks and choose **Grouping → Collapse selected into signal stack**. Existing ordinary-signal-only groups can be collapsed from **Signal stack** in the group menu. A stack shares its y-axis by design and draws every member as an equally opaque smooth curve. Its compact label legend lists every member using the exact current curve color and line pattern; hidden members remain listed in a muted style. The group flyout controls distinct colors, original colors, line patterns, member visibility, and member order. Moving a member changes which curve appears in front while its color and pattern stay with that track, including when another member is hidden. **Expand into separate tracks** restores the original track rows and their individual settings. Stack presentation is stored in workspace schema v32; v31 stacks gain stable member styles when opened, and v30 translucent-fill stacks migrate to equal-weight patterned lines.
...
- Use **Fit tracks** to fit the visible upper tracks exactly into the space above the lower pane, with no blank scroll area beneath the last track. Long names retain enough height for two centered lines. **Lock track height** in a track's context menu reserves its current height and excludes it from manual and automatic fitting; if locked or minimum heights exceed the available space, the upper tracks still scroll. The separate indicator on the right side of the button enables persistent automatic fitting as tracks or pane dimensions change. Each channel of a linked positive/negative pair receives the same height as a regular signal track.
...
In the desktop app, **File → Save workspace** writes back to the opened workspace. For a new or browser-opened workspace it opens GeRAFE's **Save workspace as…** browser first; use that command (or Ctrl+Shift+S) any time you want a copy in another location. Choose a folder, enter a `.json` file name, and confirm before replacing an existing file. GeRAFE remembers the last workspace folder independently of track folders and opens the next workspace browser there. **Open workspace…** associates the selected file with later Save operations. In a web browser, both save commands download a `.gerafe.json` export because browser security does not allow GeRAFE to overwrite a chosen local file.
...
- Cloud-file progress depends on the Windows offline/recall file attributes. GeRAFE explicitly asks Windows Cloud Files providers to hydrate the full placeholder; other providers use a sequential read. When Windows exposes locally available bytes, the in-row percentage is labeled **local**; otherwise GeRAFE falls back to bytes it has read, labeled **prepared**. Neither value is a provider-reported network-transfer percentage. If bytes stop advancing for 20 seconds, the row advises making the file available offline in Explorer. Some providers, including Dropbox's third-party placeholders, can block even an ordinary Windows file open before any bytes are returned; GeRAFE cannot make the provider finish that transfer. Check the provider's sync status, use its **Make available offline** action, and for Windows Cloud Files providers check **Automatic file downloads** permission. Large online-only genomics files may require substantial local storage and time. Browser sessions do not expose these native file attributes.
- Drag-and-drop from Windows Explorer still passes through an external shell surface, which may stall before GeRAFE receives an online-only file path. Browser-only sessions use the browser's security-controlled file chooser rather than GeRAFE's desktop file browser.
...
| `npm run version:set -- 0.2.0` | Set all application version fields together |
...
## Figure Editor (early preview)

Choose **Create figure** in GeR to start an independent FE draft with every loaded track in the same order. FE combines the upper and lower GeR track areas into one row list. Change row order with the arrows, temporarily remove or restore a row with ×/+, and edit row labels, assay labels, height, gap, color, frame, and typography in the inspector. **Back to GeR** preserves the draft; **Continue figure** reopens it. **New from view** starts over from the current GeR view after confirmation.

The figure starts with one genomic column. **Add column** duplicates the row assignments; each column can then have its own genomic region and an independently chosen loaded source track per row. This supports treatment comparisons at one locus and two-region figures. Use **Adjust region** for a wider context preview with draggable crop lines, exact coordinates, gene lookup, or saved regions. A column's ruler belongs to that column. Region linking is optional. Signal scales are shared across columns by default; row appearance controls include independent or fixed domains and per-column graph styling. Page properties also control imported region/divider/outline visibility, color, line style, and width where applicable. FE source assignments and figure styling do not edit GeR tracks.

**Save project** writes a versioned `.gerafe-figure.json` document that stores the editable layout and source-file provenance, not genomic data. On desktop, Save opens GeRAFE's Save As browser until a path exists; FE remembers its own last figure folder. Opening a project reopens unchanged local sources when possible and lists missing sources for relinking. **Export SVG** creates a vector figure; **Export PNG** re-queries source data and renders at 300 or 600 DPI, rather than enlarging a screenshot. PNG exports record their physical resolution. If source data is unavailable, export lists omissions and requires explicit confirmation. Figure projects can contain local source paths, while SVG metadata includes source names, regions, and query counts; inspect both before sharing.

This is an early figure-rendering preview, not pixel-identical browser-renderer parity. FE draws matrix colors, missing/masked pixels, BAM read marks, gene isoforms, BEDPE overlays, and matrix outlines from genomic features, but dense or specialized views can still differ from GeR. A map above 80,000 returned contact cells is omitted with an export-preflight warning; choose a coarser matrix resolution or narrower region. Inspect the exported SVG/PNG against the GeR view and source data before publication. PDF, more than two columns, callouts, reusable templates, and automatic merging of later GeR changes remain future work.
