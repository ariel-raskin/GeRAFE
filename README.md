...
- To compare ordinary BigWig, bedGraph, or TDF signals directly, select two or more compatible tracks and choose **Grouping → Collapse selected into signal stack**. Existing ordinary-signal-only groups can be collapsed from **Signal stack** in the group menu. A stack shares its y-axis by design and draws every member as an equally opaque smooth curve. Its compact label legend lists every member using the exact current curve color and line pattern; hidden members remain listed in a muted style. The group flyout controls distinct colors, original colors, line patterns, member visibility, and member order. **Expand into separate tracks** restores the original track rows and their individual settings. Stack presentation is stored in workspace schema v31; v30 translucent-fill stacks migrate to equal-weight patterned lines.
