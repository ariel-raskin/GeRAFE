...
GeRAFE reads genomic coordinates from each Cooler's stored bin table and canonicalizes the triangle orientation of both Cooler and `.hic` cells before drawing. Matrix readers and their indexes are reused across queries, and pans use an overscanned window. New tracks query the full visible genomic span by default; a bounded automatic-depth option remains available. Neither mode depends on track height, so resizing a track cannot silently multiply its contact query. Contacts are drawn directly at full canvas resolution; absent, zero, masked, or unavailable bins retain the canvas background.
