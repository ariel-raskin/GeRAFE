...
BEDPE overlays are presentation-only outlines drawn after the matrix cells and empty-state masks. They do not alter the matrix query, z-range, tiles, inspector values, or contact colors. Triangular views map same-chromosome anchor pairs into their matrix bins and respect the displayed genomic depth. Rectangular views accept either BEDPE anchor order when one anchor overlaps each axis, including trans chromosome pairs. The linked interaction track supplies its score, cis-distance, visible-gene, color, opacity, line-width, and displayed-count filters even when that arc track is hidden. Matrix-specific focus can further restrict the outline to all filtered interactions, specified gene symbols, or interactions touching a region. The matrix outline cap defaults to 250 and combines with the interaction track's displayed-count limit; a small count is drawn only when truncation occurs. Links, focus, and the cap are stored in workspace schema v25, and document pruning removes links to missing or non-interaction tracks.
